$(document).ready(function(){$(document).on('click','.product-block__gallery .product-block__gallery__item',function(){curSRC=$(this).attr('data-src');curLarge=$(this).attr('data-large');$(this).parent().find('.product-block__gallery__item').removeClass('active');$(this).addClass('active');$(this).parents('.product-preview').find('.product-preview__image > img').attr('src',curSRC).attr('data-large',curLarge)});$(document).on('click','.product-preview__info .product-params__item',function(){var $this=$(this);var param=$this.data("param");var paramValue=$this.data("paramValue");if(param&&paramValue){var $select=$('select[name='+param+']');if($select.length){$select.find(":selected").removeAttr("selected").prop("selected",!1);$select.find('option[value='+paramValue+']').attr("selected","selected").prop("selected",!0).trigger('change')}}});$(document).on('change',".js_shop_depend_param, .shop_form .depend_param",function(){select_param_price($(this).parents('form'))});function select_param_price(th)
...
{th.parents('.js_shop, .shop, .product-block, .product-preview').find('.js_shop_img, .zooms1, .shop-item-big-images a, .shop-photo img, .product-block__gallery img').each(function(){console.log($(this).attr('image_id'));if($(this).attr('image_id')==image_id)
{$(this).show();$(this).trigger('click')}
...
else{th.parents('.js_shop, .shop, .product-preview').find('.js_shop_img, .shop_img img, .shop_all_img img').each(function(){if($('.js_shop_param_price[image_id='+$(this).attr('image_id')+'], .shop_param_price[image_id='+$(this).attr('image_id')+']',th).length)
...
function depend(){var paramsActual={};var pricesActual={};var $params=$(document).find(".product-preview__info .product-params__item");$params.each(function(){var $this=$(this);if($this.data("param")){$this.removeClass("active")}});var $prices=$(document).find(".product-preview__info .product-params_price");$prices.hide().removeClass("active");$(document).find(".product-preview__info select.depend_param").each(function(){var $select=$(this);var param=$select.attr("name");var paramValue=$select.find(":selected").val();if(param&&paramValue){$params.each(function(){var $this=$(this);var p=$this.data("param");var pV=$this.data("paramValue");if(p==param&&pV==paramValue){if(!$this.is(".product-params_price")){$this.addClass("active");paramsActual[param]=paramValue}
...
return count}
$(document).on("change",".product-preview__info select.depend_param",function(){depend()})})