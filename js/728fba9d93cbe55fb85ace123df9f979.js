...
var diafan_ajax={before:{},success:{},tag:!1,init:function(form){this.tag=$("input[name=module]",form).val()+"_"+$("input[name=action]",form).val();if(this.before[this.tag]&&this.before[this.tag](form)===!1)
{return!1}
...
else{return!1}}}});$('input[type=tel]').mask('+7 (999) 999-99-99');$(".error:empty").hide();$(document).on('click','a[rel=large_image]',function(){var self=$(this);window.open(self.attr("href"),'','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width='+(self.attr("width")*1+40)+',height='+(self.attr("height")*1+40));return!1});function prepare(string){string=str_replace('&lt;','<',string);string=str_replace('&gt;','>',string);string=str_replace('&amp;','&',string);return string}
